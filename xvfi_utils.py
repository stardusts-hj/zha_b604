...
def frames_loader_test(args, I0I1It_Path, validation):
    ...
    for path in I0I1It_Path:
        ...
        frames.append(frame)
    (ih, iw, c) = frame.shape
    frames = np.concatenate(frames, axis=-1)  # (T, H, W, 3)
    ...
    """ np2Tensor [-1,1] normalized """
    # frames = RGBframes_np2Tensor(frames, args.img_ch)
    frames = RGBframes_np2Tensor_v2(frames)
...
def RGBframes_np2Tensor_v2(imgIn):
    ## input : H, W, 3C

    # to Tensor
    ts = (2, 0, 1)  ############# dimension order should be [C, T, H, W]
    imgIn = torch.Tensor(imgIn.transpose(ts).astype(float)).mul_(1.0)
